...
document.querySelector("#sehirAdı").addEventListener("submit", async (e) => {
  ...
  try {
    //let cityData = await fetch(httpRequestUrl).then((response) => response.json());
    let cityData = await vue-axios(httpRequestUrl);
    console.log(cityData.data);
    //object destructuring

    const { name, main } = cityData.data;
   ...
   // if (filteredArray.length == 1) return;
    ...
    let iconUrl = `http://openweathermap.org/img/wn/${cityData.data.weather[0].icon}@2x.png`;
    ...
    if(d==1){
      let innerHtml = `<div class="content">
        ...
        <div id="havaDurumu">Hava Durumu:${cityData.data.weather[0].description}</div>
      <img src="${iconUrl}"></div>`;
    ...
    if(d==2){
      let innerHtml = `<div class="content">
        ...
        <div id="havaDurumu">Weather:${cityData.data.weather[0].description}</div>
      <img src="${iconUrl}"></div>`;
      document.querySelector(".container").innerHTML += innerHtml;
    ...
    if(d==3){
      let innerHtml = `<div class="content">
        ...
        <div id="havaDurumu">Meteo:${cityData.data.weather[0].description}</div>
      <img src="${iconUrl}"></div>`;
      document.querySelector(".container").innerHTML += innerHtml;
